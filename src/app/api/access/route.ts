...
import { listAccess, createAccess } from '@/lib/repository/access';
...
export async function GET() {
  ...
  return withEngagement(actor.engagement!.id, async () =>
    NextResponse.json(await listAccess(actor.engagement!.id))
  );
}
...
export async function POST(req: NextRequest) {
  ...
  return withEngagement(actor.engagement!.id, async () => {
    try {
      const item = await createAccess(actor.engagement!.id, body, actor.userId);
